...
## ParameterParser.pm
##
## Parser for param.ccl
## taken from Cactus (lib/sbin/parameter_parser.pl)
##
## Copyright (C) 2013 Kurt Kanzenbach <kurt@kmk-computers.de>
##  - renamed to ParameterParser.pm
##  - made a perl module
##  - removed create_parameter_database, cross_index_parameters,
##    PrintParameterStatistics subroutines
##  - coding style
...
# exports
our @EXPORT_OK = qw(parse_param_ccl);
		...
		if ($line =~ m/(GLOBAL|RESTRICTED|PRIVATE|SHARES)\s*:\s*(\S*)\s*(.*)$/i)
		{
			...
			if ($block eq 'SHARES') {
				...
				if ($3 !~ /^\s*$/) {
					&CST_error(
						0,
						"More than one implementation on SHARES line in "
...
m/^\s*(EXTENDS\s+|USES\s+)?(?:CCTK_)?(INT|REAL|BOOLEAN|KEYWORD|STRING)\s+([a-zA-Z][a-zA-Z0-9_]*)(\s*\[([^]]+)\])?(\s+\"[^\"]*\")?\s*(.*)$/i
		  )
		{
			...
			my $variable      = $3;
			my $array_size    = $5;
			my $description   = $6 ? $6 : ""; # fix for uninitalized value
			my $options       = $7;

			$description =~ s:^\s*::;

			# fix for uninitalized value
			if ($use_or_extend) {
			...
			if ($description !~ m:\":) {
				if ($use_or_extend) {
					...
					&CST_error(
						0,
						"Missing description for parameter $variable in "
						  . "param.ccl for thorn $thorn.",
			...
			if ($options =~ m/\bAS\s+([^\s]+)\s*/i) {
				...
				if ($alias !~ m/[a-zA-Z]+[a-zA-Z0-9_]*/) {
					&CST_error(
						0,
						"Invalid alias name '$alias' for $variable of thorn "
						  . $thorn,
						'',
						__LINE__,
						__FILE__
			...
			if ($defined_parameters{"\U$variable\E"}) {
				&CST_error(
					1,
					"Duplicate parameter $variable in thorn $thorn. "
				...
				$line_number++
				  until ($line_number > @data || $data[$line_number] =~ m:\}:);
			} elsif ($use_or_extend
				&& $use_or_extend =~ m:(EXTENDS|USES):i
				&& $block !~ m:SHARES\s*\S:)
			{
				# Can only extend a friend variable.
				&CST_error(0, "Parse error in $thorn/param.ccl",
					'', __LINE__, __FILE__);
				...
				&& $use_clause == 0)
			{
				...
				&CST_error(
					0,
					"Parse error in $thorn/param.ccl - missing \"{\" in "
  ...
  #						  "$variable pf thorn $thorn", '', __LINE__, __FILE__);
				...
				foreach my $option (sort keys %options) {
					if ($option =~ m:STEERABLE:i) {
						$parameter_db{"\U$thorn $variable\E steerable"} =
						  $options{$option};
					} elsif ($option =~ m:ACCUMULATOR-BASE:i) {
						if ($options{$option} =~
							m/[a-zA-Z]+[a-zA-Z0-9_]*::([a-zA-Z]+[a-zA-Z0-9_]*)/)
						{
							if ($defined_parameters{"\U$1\E"}) {
								$parameter_db{
									"\U$thorn $variable\E accumulator-base"} =
								  $options{$option};
							} else {
								&CST_error(
									0,
									"Unknown parameter '$options{$option}' "
...
'if it comes from another thorn it must be shared',
						...
						} elsif ($options{$option} =~ m/[a-zA-Z]+[a-zA-Z0-9_]*/)
						{
							$parameter_db{
								"\U$thorn $variable\E accumulator-base"} =
							  "$thorn\::$options{$option}";
						} else {
							&CST_error(
								0,
"Invalid accumulator-base $options{$option} for "
						...
						if ($retcode == 0) {
							$parameter_db{
								"\U$thorn $variable\E accumulator-expression"}
							  = $options{$option};
						} elsif ($retcode == 1) {
							&CST_error(
								0,
								"Invalid accumulator expression "
							...
							&CST_error(
								0,
								"Arithmetic error in accumulator expression "
								  . "'$options{$option}' for $variable of thorn $thorn",
								'',
								__LINE__,
								__FILE__
							...
							&CST_error(
								0,
								"Accumulator expression '$options{$option}' "
							...
							&CST_error(
								0,
								"Internal-error while checking accumulator "
						...
						&CST_error(
							0,
							"Unknown option '$option' for parameter $variable "
							  . "of thorn $thorn",
							'',
							__LINE__,
							__FILE__
				...
				if ($array_size) {
					if ($array_size !~ /^\d+$/) {
						&CST_error(
							0,
							"Invalid array size '$array_size' for parameter "
							  . "$variable of thorn $thorn",
							'',
							__LINE__,
							__FILE__
						...
						$parameter_db{"\U$thorn $variable\E array_size"} =
						  $array_size;
					}
				}
			   ...
			   # Strip out double quotes enclosing a non-numeric parameter range
						  ...
						  )
						{
							&CST_error(
								0,
								"Invalid range '$new_ranges' for integer "
						...
						$parameter_db{
"\U$thorn $variable\E range $parameter_db{\"\U$thorn $variable\E ranges\"} range"
						} = $new_ranges;
							...
							|| ($delim =~ /::/ && $new_desc =~ /^\s*$/))
						{
							...
							&CST_error(
								1,
"Missing description for range $1 for parameter "
...
"Each parameter range line should have the syntax "
							...
							/^\s*\".*[^\s\"]\s*$|^\s*[^\s\"].*\"\s*$/)
						{
							&CST_error(
								0,
								"Description of range for parameter $variable "
						...
						$parameter_db{
"\U$thorn $variable\E range $parameter_db{\"\U$thorn $variable\E ranges\"} description"
						} = $new_desc;
						$line_number++;
					}
				}
				  ...
				  )
				{
					&CST_error(
						0,
						"No range provided for parameter $variable in "
						  . "param.ccl for thorn $thorn",
				...
				if ($block !~ m:SHARES:) {
					if ($data[$line_number] =~ m:\s*\}\s*([^\s].*)\s*:) {
						...
						if ($type eq 'INT' && $default !~ /^[+-]?\d+$/) {
							&CST_error(
								0,
								"Default '$default' for integer parameter "
							...
							/^[+-]?(\d+([.]\d*)?|[.]\d+)([eEdD][+-]?\d+)?$/)
						{
							&CST_error(
								0,
								"Default '$default' for real parameter "
						...
						if ($type =~ m:INT|REAL: && $default =~ m:":) {
							&CST_error(
								0,
								"String default given for $type $variable in "
								  . "$thorn",
								'',
								__LINE__,
								__FILE__
							...
							&CST_error(
								0,
								"Default given for $type $variable in $thorn "
								  . "is not a string",
								'',
								__LINE__,
								__FILE__
						...
						&CheckParameterDefault($thorn, $variable, $default,
							%parameter_db);

						$parameter_db{"\U$thorn $variable\E default"} =
						  $default;
					} else {
						&CST_error(
							0,
							"Unable to find default value for parameter "
							  . "$variable",
				...
				if ($block =~ m:SHARES:i) {
					if ($data[$line_number] =~ m:\s*\}\s*([^\s].*)\s*:) {
						&CST_error(
							0,
							"param.ccl of thorn '$thorn' attempts to change "
			...
			if ($line =~ m:\{:) {
				&CST_error(1,
					"Skipping parameter block in $thorn with missing keyword",
					'', __LINE__, __FILE__);
				$line_number++ until ($data[$line_number] =~ m:\}:);
			} else {
				$line =~ /^(.*)\n*$/;
				&CST_error(0,
					"Unknown line in param.ccl for thorn $thorn\n\"$1\"",
					'', __LINE__, __FILE__);
			}
		}
	}

	$parameter_db{"\U$thorn\E SHARES implementations"} =
	  join(" ", sort keys %friends);
...
sub CheckParameterDefault
{
	...
	my ($foundit, $i, $range, $nranges);
	my ($lower_bounds_excluded, $upper_bounds_excluded, $min, $max);
	...
	if ($parameter_db{"\U$thorn $variable\E type"} =~ /BOOLEAN/) {
		if ($default !~ m:^yes|no|y|n|1|0|t|f|true|false$:i) {
			&CST_error(
				0,
				"Default ($default) for boolean parameter '$variable' "
		...
		if ($foundit == 0) {
			&CST_error(
				0,
				"Default ($default) for keyword parameter '$variable' "
				  . "is incorrect in param.ccl for thorn $thorn",
		...
		if ($foundit == 0) {
			&CST_error(
				0,
				"Default ($default) for string parameter '$variable' "
				  . "is incorrect in param.ccl for thorn $thorn",
		...
		if ($nranges > 0 && $foundit == 0) {
			&CST_error(
				0,
				"Default ($default) for integer parameter '$variable' "
				  . "is incorrect in param.ccl for thorn $thorn",
				"The default value for a parameter must lie within the "
				  . "allowed range",
				__LINE__,
		...
		if ($nranges > 0 && $foundit == 0) {
			&CST_error(
				0,
				"Default ($default) for real parameter '$variable' "
				  . "is incorrect in param.ccl for thorn $thorn",
				"The default value for a parameter must lie within the "
				  . "allowed range",
				__LINE__,
...
sub CheckExpression
{
		...
		&& $expression !~ m/y\w/)
	{
		...
		# Calculate L(L(a,b),c).
		my $answer1 = &EvalExpression(&EvalExpression($a, $b, "$expression"),
			$c, $expression);

		#	 print "$answer1\n" if defined $answer1;

		# Calculate L(L(a,c),b).
		my $answer2 = &EvalExpression(&EvalExpression($a, $c, "$expression"),
			$b, $expression);
...
sub EvalExpression
{
	...
	# changed to { } form of eval, see page 161 of PBP
