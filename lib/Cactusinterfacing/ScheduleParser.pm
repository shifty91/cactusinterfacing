...
##
## ScheduleParser.pm
## taken from Cactus (lib/sbin/ScheduleParser.pl)
##
## Copyright (C) 2013 Kurt Kanzenbach <kurt@kmk-computers.de>
##  - renamed to ScheduleParser.pm
##  - made a perl module
##  - added use strict, warnings
##  - coding style
##  - removed PrintScheduleStatistics, check_schedule_database,
##    create_schedule_database subroutines
##  - changed scope from schedule_bins and schedule_bin_regexp
##

package Cactusinterfacing::ScheduleParser;
...
# export
our @EXPORT_OK = qw(parse_schedule_ccl);
...
my $schedule_bin_regexp = 'CCTK_(' . join('|', @schedule_bins) . ')';
...
sub parse_schedule_ccl {
	...
	my ($n_statements);
	my (
		$name,           $as,          $type,        $description,
		$where,          $language,    $mem_groups,  $comm_groups,
		$trigger_groups, $sync_groups, $options,     $tags,
		$before_list,    $after_list,  $writes_list, $reads_list,
		$while_list,     $if_list
	);
	my ($groups);
	my ($hint, $message);
	...
	for ($line_number = 0 ; $line_number < scalar(@data) ; $line_number++) {
		if ($data[$line_number] =~ m:^\s*schedule\s*:i) {
			(
				$line_number, $name,           $as,          $type,
				$description, $where,          $language,    $mem_groups,
				$comm_groups, $trigger_groups, $sync_groups, $options,
				$tags,        $before_list,    $after_list,  $writes_list,
				$reads_list,  $while_list,     $if_list
			...
			$schedule_db{"\U$thorn\E BLOCK_$n_blocks DESCRIPTION"} =
			  $description;
			...
			($line_number, $type, $groups) =
			  &ParseScheduleStatement($line_number, @data);
			...
			$message =
"Format error in STORAGE statement of $thorn\nLine is: $data[$line_number]";
...
sub ParseScheduleBlock {
	...
	while ($field <= $#fields) {
		...
		if ($fields[$field] =~ m:^AT$:i) {
			...
			if ($where ne "") {
				print STDERR
				  "Error parsing schedule block line '$data[$line_number]'\n";
				print STDERR
				  "Attempt to schedule same block at/in two places.\n";
			} else {
			...
			if ($where !~ $schedule_bin_regexp) {
				&CST_error(
					0,
					"Schedule bin \'$where\' not recognised in schedule.ccl "
					  . "file of thorn $thorn",
					"",
					__LINE__,
					__FILE__
...
"Error parsing schedule block line '$data[$line_number]'\nMissing { at start of block",
		...
		$line_number++
		  while ($line_number < scalar(@data)
			and $data[$line_number] !~ m:\s*\}\s*:);
	} else {
		while ($data[$line_number] !~ m:\s*\}\s*:) {
			...
			if ($data[$line_number] =~ m/^\s*STOR[^:]*:\s*(.*)$/i) {
				if ($where eq "CCTK_STARTUP") {
					&CST_error(
						1,
"Scheduling storage \"$name\" at startup in thorn \"$thorn\"",
						"Storage cannot be allocated at startup",
						__LINE__,
						__FILE__
					);
					...
					&CST_error(
						1,
"Scheduling storage \"$name\" at shutdown in thorn \"$thorn\"",
						"Storage cannot be allocated at shutdown",
						__LINE__,
						__FILE__
					);
				}
				...
				if ($language ne "") {
					...
					if ($type eq "GROUP") {
						&CST_error(
							1,
"Scheduling group \"$name\" with LANG specifier in thorn \"$thorn\"",
							"Groups should not have a LANG specificier",
							__LINE__,
							__FILE__
						);
					}
				...
				&CST_error(
					0,
"Unrecognised statement in schedule block ($name) in schedule.ccl for thorn $thorn\n\"$1\"",
					"",
					__LINE__,
					__FILE__
				);
	...
	if ($data[$line_number] =~ m:\s*\}\s*\"([^\"]*)\":) {
		...
		$message =
"Missing desciption at end of schedule block ($name) in schedule.ccl for thorn $thorn";
		&CST_error(0, $message, "", __LINE__, __FILE__);
	}
